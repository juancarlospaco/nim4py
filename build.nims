...
const version = "1.0"
const packageName = "nimscript4python"
...
withDir("dist"):
  ...
  withDir("lin"):
    selfExec "compileToC --nimcache:. " & rootFolder / "src/nimscript4python.nim"
    rmFile(packageName & ".json")
    exec "rm --force *.c.o"
    cpFile(nimbaseH, "nimbase.h")

  withDir("win"):
    selfExec "compileToC --nimcache:. --os:windows --gcc.exe:" & gccWin32 & " --gcc.linkerexe:" & gccWin32 & " " & rootFolder / "src/nimscript4python.nim"
    rmFile(packageName & ".json")
    exec "rm --force *.c.o"
    cpFile(nimbaseH, "nimbase.h")
